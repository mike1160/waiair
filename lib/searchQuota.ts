...
export const FREE_LIFETIME_SEARCHES = 10;
...
/** Free: 10 lifetime. Credits: 50 a day. Pro: 100 a day. */
export function searchLimit(tier: SearchTier): { period: 'lifetime' | 'day'; max: number } {
  if (tier === 'pro') return { period: 'day', max: PRO_DAILY_SEARCHES };
  if (tier === 'credits') return { period: 'day', max: CREDITS_DAILY_SEARCHES };
  return { period: 'lifetime', max: FREE_LIFETIME_SEARCHES };
}
...
/** AsyncStorage key of the tier's ledger: the lifetime free count, or today's daily count. */
export function searchLedgerKey(tier: SearchTier, day: string): string {
  return searchLimit(tier).period === 'lifetime' ? SEARCH_COUNT_FREE_KEY : dailySearchKey(day);
}
