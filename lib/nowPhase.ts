...
export function nowPhaseId(opts: {
  ...
  const m = typeof opts.minutesToDeparture === 'number' && Number.isFinite(opts.minutesToDeparture)
    ? opts.minutesToDeparture
    : null;
...
export function nowPhaseLines(id: NowPhaseId, copy: NowPhaseCopy, facts: NowPhaseFacts = {}): { title: string; sub: string } {
  ...
  switch (id) {
    case 'tomorrow':
      return { title: copy.nowTomorrow, sub: copy.nowTomorrowSub(hoursUntilCheckin(facts.minutesToDeparture)) };
    case 'checkin':
