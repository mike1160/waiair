/**
 * Gate and terminal walking times, arrival hints and connection bands. Pure — no i18n, no React Native — so
 * lib/tripMoments.ts can use it under `node --test`. The two helpers that turn a result into translated copy
 * stay in ./gateWalk.ts, which re-exports everything here so existing imports keep working.
 */

import { getTerminalWalkTime, normalizeTerminalCode } from './terminalWalkTimes.ts';
