/**
 * Credits account: Sign in with Apple / Google, proxy session (SecureStore) and the /credits endpoints.
 * The proxy verifies the provider ID token and holds the RevenueCat secret key; the app only sees its session.
 */
...
export type CreditProvider = 'apple' | 'google';
...
function parseSession(raw: string | null): CreditSession | null {
  ...
  try {
    ...
    if (!s.userId || !s.sessionToken || (s.provider !== 'apple' && s.provider !== 'google')) return null;
...
/**
 * Sign in and exchange the provider ID token for a proxy session. `deviceFreeUsed` carries free flights
 * used while signed out. Returns null when the user cancels.
 */
export async function signInForCredits(provider: CreditProvider, deviceFreeUsed: number): Promise<CreditSession | null> {
  ...
  const res = await fetchWithTimeout(`${PROXY}/credits/session`, {
    ...
    body: JSON.stringify({ provider, idToken, freeUsed: deviceFreeUsed }),
  }, REQUEST_TIMEOUT_MS);
...
export async function signOutProvider(provider: CreditProvider | null | undefined): Promise<void> {
  if (provider !== 'google' || !isGoogleSignInConfigured()) return;
  try {
